 ...
 * <P>
 * The applet parameters that can be specified for this
 * applet are "SCALE", "MODE" and "SNAPSHOT", to provide the
 * screen scale, hardware mode and, respectively, the snapshot
 * file to load into the emulator.
 * <P>
 * The applet width and height must be a multiple of
 * (316, 252)
 *
...
public class JZXApplet extends Applet implements Runnable {
	...
	public String[][] getParameterInfo() {
		return new String[][]{
				...
				{SNAPSHOT_PARAMETER, "URI", "Codebase-relative URI to Z80 snapshot"}
		};
	}
	
	/**
	 * Parse the parameters, load the snapshot, create the spectrum
	 * emulator instance, pause it and start a runner thread to
		...
		if (snapshot != null) {
			try {
				...
				if (loader.getMode() == BaseLoader.MODE_48) {
					m_spectrum = new org.razvan.jzx.v48.Spectrum();
				} else if (loader.getMode() == BaseLoader.MODE_128) {
					m_spectrum = new org.razvan.jzx.v128.Spectrum();
				} else {
					logger.log(ILogger.C_ERROR, "Unknown hardware mode: " + loader.getMode());
				}
	...
	/**
	 * This is called from the thread started in <TT>init()</TT>
	 * and simply calls the <TT>emulate()</TT> method of the
	 * emulator.
	 *
	 * @see #init
	 * @see #start
	...
	/**
	 * Unpause the emulator which is current paused in the
	 * runner thread.
	 *
	 * @see #init
	 * @see #run
	 ...
	 * Stop the emulator.
	 * <P>
	 * This will cause the <TT>run()</TT> method above
	 * to return, which will end the runner thread. To
	 * restart the emulator, the <TT>init()</TT> method
