 ...
 * application.
 * <P>
 * The command line parameters that can be specified for this
 * application are "-scale", "-mode" and "-snapshot", to provide
 * the screen scale, hardware mode and, respectively, the snapshot
...
public class JZXFrame {
	...
	/**
	 * Search the parameter list for a specified parameter and return
	 * the next entry in the parameter list.
	 * <P>
	 * The method returns null if the parameter is not found or if
	...
	/**
	 * Create the Frame, create the emulator instance,
	 * load the snapshot (if appropriate) and start
	 * the emulation loop.
	 */
		...
		if (snapshot != null) {
			try {
				...
				if (loader.getMode() == BaseLoader.MODE_48) {
					spectrum = new org.razvan.jzx.v48.Spectrum();
				} else if (loader.getMode() == BaseLoader.MODE_128) {
					spectrum = new org.razvan.jzx.v128.Spectrum();
				} else {
