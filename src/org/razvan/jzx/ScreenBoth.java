 ...
 * <P>
 * This implementation uses a MemoryImageSource, and calls
 * the method <TT>newPixels()</TT> to indicate that new data
 * is available for drawing into the image.
 *
...
public class ScreenBoth extends BaseScreen {
	...
	/**
	 * The underlying RGB data for the MemoryImageSource
	 * object.
	 */
	private int[] m_data;
	
	/**
	 * The MemoryImageSource object whose data is updated
	 * and drawn on the screen.
	 */
	private MemoryImageSource m_memoryImageSource;
	
	/**
	 * Allocate the RGB data array and the MemoryImageSource
	 * object that Decorates the data.
	 */
	public void init(BaseSpectrum spectrum, ILogger logger) {
		...
		m_memoryImageSource = new MemoryImageSource(m_screenWidth, m_screenHeight,
				m_data, 0, m_screenWidth);
		m_memoryImageSource.setAnimated(true);
	}
	
	/**
	 * Update the RGB data array and the MemoryImageSource for
	 * the new scale value.
	 */
	...
	/**
	 * Create the MemoryImageSource object from the given
	 * Graphics object.
	 */
	...
	/**
	 * Draw a given byte into the RGB array using the given
	...
	protected void draw8(int x, int y, int pix8, int attr8) {
		...
		for (int i = 0; i < 8 * m_scale;) {
			int rgb = (0xff000000 | s_rgbPalette[((pix8 & 0x80) != 0 ? ipix : ppix)]);
		...
		for (int i = 1; i < m_scale; i++) {
			System.arraycopy(m_data, start, m_data, (start + i * m_screenWidth), 8 * m_scale);
		}
	}
	
	/**
	 * Call <TT>newPixels()</TT> on the MemoryImageSource object
