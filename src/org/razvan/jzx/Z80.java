...
public class Z80 extends BaseComponent {
	/**
	 * Indicates that the <TT>emulate()</TT>
	 * method should end.
	 */
	 ...
	 * <P>
	 * This count is incremented by the appropriate
	 * amount for every instruction that is executed.
	 * <P>
	 * The T-States are used for synchonizing the
	 * rendering of screen lines that comprise the
	 * screen frame.
	 */
	 ...
	 * <P>
	 * This is used to read and write data as
	 * CPU instructions access the memory.
	 */
	 ...
	 * <P>
	 * This is used to read and write I/O ports
	 * CPU instructions are decoded.
	 */
	private BaseIO m_io;
	...
	/**
	 * Explicit flags, which represent the bits in the F register
	 * as distinct variables.
	 *
	 * @see #storeFlags
	 * @see #retrieveFlags
	 */
	private boolean m_carryF, m_addsubtractF, m_parityoverflowF, m_halfcarryF, m_zeroF, m_signF, m_5F, m_3F;
	 ...
	 * 
	 * ADD HL,xx      = high byte of HL, i.e. H, before the addition
	 * LD r,(IX/IY+d) = high byte of the resulting address IX/IY+d
	 * JR d           = high byte target address of the jump
	 */
	private int m_x8;
	 ...
	 * Interrupt the CPU.
	 * <P>
	 * If IFF1a is not zero, set both IFF flags to 0 and push
	 * the value of PC on the stack. If the IM is 2, jump to
	 * the address (I, 0xFF), otherwise jump to 0x38.
	 */
	 ...
	 * <P>
	 * Set IFF1b = IFF1a, and set IFF1a to 0 (disable
	 * regular interrupts.) Push the PC on the stack,
	 * then jump to address 0x66.
	 */
	public void nmi() {
		m_tstates += 15;
	...
	/**
	 * Accessor for the 16-bit AF register (it combines the
	 * A and F registers.)
	 */
	private int af16() {
		return ((m_a8 << 8) | m_f8);
	...
	/**
	 * Accessor for the 16-bit BC register (it combines the
	 * A and F registers.)
	 */
	private int bc16() {
		return ((m_b8 << 8) | m_c8);
	...
	/**
	 * Accessor for the 16-bit DE register (it combines the
	 * A and F registers.)
	 */
	private int de16() {
		return ((m_d8 << 8) | m_e8);
	...
	/**
	 * Accessor for the 16-bit HL register (it combines the
	 * A and F registers.)
	 */
	private int hl16() {
		return ((m_h8 << 8) | m_l8);
	...
	/**
	 * Mutator for the 16-bit AF register (it sets the
	 * A and F registers.)
	 */
	...
	/**
	 * Mutator for the 16-bit BC register (it sets the
	 * A and F registers.)
	 */
	...
	/**
	 * Mutator for the 16-bit DE register (it sets the
	 * A and F registers.)
	 */
	...
	/**
	 * Mutator for the 16-bit HL register (it sets the
	 * A and F registers.)
	 */
	...
	/**
	 * Accessor for the low 8 bits of the current index
	 * register.
	 * <P>
	 * The current index register is stored in m_xx16,
	 * and could either be m_ix16 or m_iy16.
	 */
	...
	/**
	 * Accessor for the high 8 bits of the current index
	 * register.
	 * <P>
	 * The current index register is stored in m_xx16,
	 * and could either be m_ix16 or m_iy16.
	...
	/**
	 * Mutator for the low 8 bits of the current index
	 * register.
	 * <P>
	 * The current index register is stored in m_xx16,
	 * and could either be m_ix16 or m_iy16.
	...
	/**
	 * Mutator for the high 8 bits of the current index
	 * register.
	 * <P>
	 * The current index register is stored in m_xx16,
	 * and could either be m_ix16 or m_iy16.
	...
	/**
	 * Store the values from m_*F variables into the m_f8
	 * register, according to the *_MASK variables.
	 */
	private void storeFlags() {
		if (m_signF) m_f8 |= SIGN_MASK; else m_f8 &= ~SIGN_MASK;
		if (m_zeroF) m_f8 |= ZERO_MASK; else m_f8 &= ~ZERO_MASK;
		if (m_halfcarryF) m_f8 |= HALFCARRY_MASK; else m_f8 &= ~HALFCARRY_MASK;
		if (m_parityoverflowF) m_f8 |= OVERFLOW_MASK; else m_f8 &= ~OVERFLOW_MASK;
		if (m_addsubtractF) m_f8 |= ADDSUBTRACT_MASK; else m_f8 &= ~ADDSUBTRACT_MASK;
		if (m_carryF) m_f8 |= CARRY_MASK; else m_f8 &= ~CARRY_MASK;
		if (m_3F) m_f8 |= THREE_MASK; else m_f8 &= ~THREE_MASK;
		if (m_5F) m_f8 |= FIVE_MASK; else m_f8 &= ~FIVE_MASK;
	}
	
	/**
	 * Retrieve the flags from the m_f8 register into the
	 * the m_*F variables, according to the *_MASK variables.
	 */
	...
	/**
	 * Add a 16-bit value to the (current) index register (m_xx16)
	 * and set the appropriate flags.
	 */
	private void add_xx(int val16) {
		int work32 = m_xx16 + val16;
		int idx = ((m_xx16 & 0x800) >> 9) | ((val16 & 0x800) >> 10) | ((work32 & 0x800) >> 11);
	...
	/**
	 * Add a 16-bit value to the HL register
	 * and set the appropriate flags.
	 */
	private void add_hl(int val16) {
		...
		int idx = ((hl16 & 0x800) >> 9) | ((val16 & 0x800) >> 10) | ((work32 & 0x800) >> 11);
		hl16(work32 & 0xffff);
		m_halfcarryF = m_halfcarryTable[idx];
		m_addsubtractF = false;
		m_carryF = ((work32 & 0x10000) != 0);
	...
	/**
	 * Add a 16-bit value, with carry, to the HL register
	 * and set the appropriate flags.
	 */
	private void adc_hl(int val16) {
		int hl16 = hl16();
		int work32 = hl16 + val16 + (m_carryF ? 1 : 0);
		int idx = ((hl16 & 0x8800) >> 9) | ((val16 & 0x8800) >> 10) | ((work32 & 0x8800) >> 11);
		hl16 = (work32 & 0xffff);
		hl16(hl16);
	...
	/**
	 * Subtract a 16-bit value, with carry, from the HL register
	 * and set the appropriate flags.
	 */
	...
	/**
	 * Add two 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int add16(int reg16, int val16) {
		return (reg16 + val16) & 0xffff;
	}
	
	/**
	 * Subtract two 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int sub16(int reg16, int val16) {
		return (reg16 - val16) & 0xffff;
	}
	
	/**
	 * Increment a 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int inc16(int reg16) {
		return (reg16 + 1) & 0xffff;
	}
	
	/**
	 * Increment twice a 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int incinc16(int reg16) {
		return (reg16 + 2) & 0xffff;
	}
	
	/**
	 * Decrement a 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int dec16(int reg16) {
		return (reg16 - 1) & 0xffff;
	}
	
	/**
	 * Decrement twice a 16-bit values and return the 16-bit result,
	 * without setting any flags.
	 */
	private int decdec16(int reg16) {
		return (reg16 - 2) & 0xffff;
	}	
	
	/**
	 * Increment a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int inc8(int reg8) {
	...
	/**
	 * Decrement a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int dec8(int reg8) {
		int work8 = (reg8 - 1) & 0xff;
		m_signF = ((work8 & 0x80) != 0);
		m_zeroF = (work8 == 0);
	...
	/**
	 * Rotate left with carry a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int rlc8(int reg8) {
	...
	/**
	 * Rotate left a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int rl8(int reg8) {
	...
	/**
	 * Rotate right a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int rr8(int reg8) {
	...
	/**
	 * Shift left a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int sla8(int reg8) {
		m_carryF = ((reg8 & 0x80) != 0);
		int work8 = (reg8 << 1) & 0xff;
		shift_test(work8);
		return work8;
	...
	/**
	 * Shift right a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int sra8(int reg8) {
		m_carryF = ((reg8 & 0x01) != 0);
		int work8 = (reg8 & 0x80);
		work8 = ((reg8 >> 1) | work8);
		shift_test(work8);
	...
	/**
	 * Shift left with 1-pad a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int sli8(int reg8) {
		m_carryF = ((reg8 & 0x80) != 0);
	...
	/**
	 * Shift right with 1-pad a 8-bit value and return the 8-bit result,
	 * setting the appropriate flags.
	 */
	private int srl8(int reg8) {
		m_carryF = ((reg8 & 0x01) != 0);
		int work8 = (reg8 >> 1);
		shift_test(work8);
		return work8;
	 ...
	 */
	private void shift_test(int reg8) {
		m_signF = ((reg8 & 0x80) != 0);
		m_zeroF = (reg8 == 0);
		m_halfcarryF = false;
	...
	/**
	 * Pop the current 16-bit value at the top of the stack and
	 * return it.
	 */
	private int pop16() {
	 ...
	 */
	private void push(int reg16) {
		m_sp16 = decdec16(m_sp16);
		m_memory.write16(m_sp16, reg16);
	}
	
	private void ld_a_special(int reg8) {
		m_a8 = reg8;
		m_signF = ((m_a8 & 0x80) != 0);
		m_zeroF = (m_a8 == 0);
		m_halfcarryF = false;
	...
	/**
	 * Add a 8-bit value to the A register and set the appropriate
	 * flags.
	 */
	private void add_a(int val8) {
		int work16 = m_a8 + val8;
		int idx = ((m_a8 & 0x88) >> 1) | ((val8 & 0x88) >> 2) |	((work16 & 0x88) >> 3);
		m_a8 = work16 & 0xff;
		m_signF = ((m_a8 & 0x80) != 0);
		m_zeroF = (m_a8 == 0);
		m_halfcarryF = m_halfcarryTable[idx & 0x7];
	...
	/**
	 * Subtract a 8-bit value from the A register and set the appropriate
	 * flags.
	 */
	...
	/**
	 * Subtract with carry a 8-bit value from the A register and set the appropriate
	 * flags.
	 */
	...
	/**
	 * And a 8-bit value to the A register and set the appropriate
	 * flags.
	 */
	...
	/**
	 * Xor a 8-bit value to the A register and set the appropriate
	 * flags.
	 */
	...
	/**
	 * Or a 8-bit value to the A register and set the appropriate
	 * flags.
	 */
	...
	/**
	 * Compare a 8-bit value to the A register and set the appropriate
	 * flags.
	 */
	 ...
	 * <P>
	 * The R register is a counter that is updated every
	 * instruction, where DD, FD, ED and CB are to be
	 * regarded as separate instructions. So shifted
	 * instruction will increase R by two. There's an
	 * interesting exception: doubly-shifted opcodes, the
	 * DDCB and FDCB ones, increase R by two too. LDI
	 * increases R by two, LDIR increases it by 2 times BC,
	 * as does LDDR etcetera. The sequence LD R,A/LD A,R
	 * increases A by two, except for the highest bit: this
	 * bit of the R register is never changed.
	 */
	private int mone8() {
	...
	/**
	 * Decode one instruction: call <TT>mone8()</TT>
	 * to retrieve the opcode, decode it and execute it.
	 */
	public void emulate() {		
		while(true) {
			m_spectrum.update();
			...
			case 0x08:
				m_tstates += 4;
				storeFlags();
				work16 = af16(); af16(m_af16alt); m_af16alt = work16;
				retrieveFlags();
				break;
			...
			case 0x27:
				...
				if (!addsubtract) {
					...
					if (m_carryF || (m_a8 >> 4) > 9 || ((m_a8 >> 4) >= 9 && (m_a8 & 0x0f) > 9)) {
						work8 |= 0x60;
						carry = true;
					}
				} else {
			...
			case 0xD9:
				m_tstates += 4;
				work16 = bc16(); bc16(m_bc16alt); m_bc16alt = work16;
				work16 = de16(); de16(m_de16alt); m_de16alt = work16;
				work16 = hl16(); hl16(m_hl16alt); m_hl16alt = work16;
				break;
			...
			case 0xEB:
				m_tstates += 4;
				work16 = de16(); de16(hl16()); hl16(work16);
				break;
	...
	private void decodeCB(int op8) {
		...
		case 0x2F:
			 ...
			 * 
			 * These instructions shift left the operand
	 ...
	 * <P>
	 * This method only operates on the m_xx16 variable,
	 * following that the caller of this method will
	 * assign m_xx16 to the appropriate index register
	 * (m_ix16 or m_iy16).
	 */
	public void decodeXX(int op8) {
		...
		case 0x36:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(m_pc16)), m_memory.read8(inc16(m_pc16)));
			m_pc16 = incinc16(m_pc16);
			break;
		...
		case 0x70:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_b8);
			break;
			
			/* ld (xx+d),c */
		case 0x71:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_c8);
			break;
			
			/* ld (xx+d),d */
		case 0x72:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_d8);
			break;
			
			/* ld (xx+d),e */
		case 0x73:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_e8);
			break;
			
			/* ld (xx+d),h */
		case 0x74:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_h8);
			break;
			
			/* ld (xx+d),l */
		case 0x75:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_l8);
			break;
			
			/* ld (xx+d),a */
		case 0x77:
			m_tstates += 19;
			m_memory.write8(add16(m_xx16, (byte)m_memory.read8(inc16pc())), m_a8);
			break;
		...
		case 0x86:
			m_tstates += 19;
			work8 = m_memory.read8(add16(m_xx16, (byte)m_memory.read8(inc16pc())));
			add_a(work8);
			break;
		...
		case 0xF9:
			...
			/**
			 * Many DD or FD opcodes after each other will
			 * effectively be NOPs, doing nothing except
			 * repeatedly setting the flag "treat HL as IX"
			 * (or IY) and taking up 4 T states 
			 */
		...
		m_logger.log(ILogger.C_ERROR, "Unimplemented instruction: " +
				m_memory.read8(dec16(m_pc16)) + " " +
				m_memory.read8(m_pc16) + " at " + 
				dec16(m_pc16));
		break;
		}
	}
	...
	private void decodeED(int op8) {
		...
		case 0x7d:	/* unofficial */
			...
			/**
			 * The IM 0/1 instruction puts the processor in either
			 * IM 0 or 1 (undetermined at this time). The SGS booklet 
			 * says that there are two flip-flops which determine the 
			 * interrupt mode and are set by the IM instructions. The 
			 * possible values are:
			 * 
			 * 00  IM 0
			 * 01  not used
			 ...
			 * 11  IM 2
			 * 
			 * so it is entirely possible that the IM 0/1 instruction 
			 * sets these registers to 01. What the Z80 does when this 
		...
		case 0x6F:
			...
			/**
			 * The ED70 instruction reads from port (C), just like
			 * the other instructions, but throws away the result. 
			 * It does change the flags in the same way as the other 
			 * IN instructions, however. 
			 */
		...
		case 0xFD:
			...
			/**
			 * The ED opcodes in the range 00-3F and 80-FF (except 
			 * for the block instructions of course) do nothing at 
			 * all but taking up 8 T states and incrementing the R
			 * register by 2. 
			 */
	...
	/**
	 * Return a string representation of the CPU state, 
	 * which is useful for debugging purposes.
	 */
	public String toString() {
		storeFlags();
		
		return "A=" + m_a8 + ",F=" + m_f8 +
		",B=" + m_b8 + ",C=" + m_c8 +
		",D=" + m_d8 + ",E=" + m_e8 +
		",H=" + m_h8 + ",L=" + m_l8 +
		",AF1=" + m_af16alt + ",BC1=" + m_bc16alt +
		",DE1=" + m_de16alt + ",HL1=" + m_hl16alt + 
		",IX=" + m_ix16 + ",IY=" + m_iy16 +
		",SP=" + m_sp16 + ",PC=" + m_pc16 +
		",R=" + m_r8 + ",I=" + m_i8 +
		",IM=" + m_im2 + ",IFF1=" + m_iff1a + ",IFF2=" + m_iff1b + ",OP=" + m_memory.read8(m_pc16);
	}	
