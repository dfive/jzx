 ...
 *         me.
 * 
 * @see pt.ist.ave.jzx.v48.IO
 * @see pt.ist.ave.jzx.v128.IO
 */
public class BaseIO extends BaseComponent {
	 ...
	 * <P>
	 * This is used for all memory page modifications that are triggered by I/O
	 * instructions.
	 */
	...
	public void init(BaseSpectrum spectrum, ILogger logger) {
		...
		if (BaseSpectrum.FREQUENCY_MS > 1) {
			AudioFormat format = new AudioFormat(
					AudioFormat.Encoding.PCM_UNSIGNED, // encoding (pulse code
														// modulation)
					BaseIO.SAMPLE_FREQ, // sample rate: number of samples taken
										// per second, per channel
					8, // sample size in bits
					1, // channels (1 = mono, 2 = stereo)
					1, // frame size: number of channels * sample size per
						// channel (in bytes)
					BaseIO.SAMPLE_FREQ, // frame rate: number of frames (set of
										// samples for all channels) taken per
										// second
					true); // big endian; relevant only if sample size > 8

			// It's good to set the recommended line buffer size to be a
			// multiple of
			// the sample buffer size (instead of exactly the sample buffer
			// size),
			// otherwise you introduce crackles when buffers are played.
			// Presumably,
			...
			DataLine.Info info = new DataLine.Info(SourceDataLine.class,
					format, 2 * BaseIO.LINE_BUF_SIZE);

			if (!AudioSystem.isLineSupported(info)) {
				m_logger.log(ILogger.C_DEBUG, "Unsupported audio format: "
						+ format);
			}
	...
	public int in8(int port16) {
		...
		if ((port16 & B_ULA) == 0) {
			...
			// are one.
			int res8 = (m_spectrum.getIssue() == BaseSpectrum.ISSUE_2) ? 0xff
					: 0xdf;
		...
		return (vline < 192 ? m_memory.read8((BaseScreen.ATTR_START)
				| ((vline & 0xf8) << 2)) : 0xff);
	}
