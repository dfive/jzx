...
public abstract class BaseScreen extends Canvas {
	 ...
	 */
	protected static final Color[] s_colorPalette = { new Color(0x000000), /* black */
	 ...
	 */
	protected static final int[] s_rgbPalette = { 0x000000, /* black */
	0x0000bf, /* blue */
	0xbf0000, /* red */
	0xbf00bf, /* magenta */
	0x00bf00, /* green */
	 ...
	 * <P>
	 * This is used to read the actual screen data in order to draw it on the
	 * screen.
	 */
	 ...
	 * <P>
	 * This method updates all the screen size parameters in accordance to the
	 * new scale value. This method should be appropriately overriden by child
	 * classes in order to to modify their inner structures if the scale is
	 * changed.
	 */
	 ...
	 * flicker.
	 * 
	 * This method just calls {@link #paint} with the specified Graphics
	 * context.
	 */
	...
	public void paint(Graphics g) {
		...
		if (m_offscreenImage == null) {
			m_offscreenImage = createCompatibleImage(m_screenWidth,
					m_screenHeight);
		...
		if (m_screenDirty) {
			...
			for (int addr16 = 0; addr16 < PIXEL_LENGTH; addr16++) {
				...
				int attr8 = ((int) memory[PIXEL_LENGTH
						+ ((x >> 3) + ((y & 0xf8) << 2))]) & 0xff;
			...
			m_offscreenBorderGraphics.drawImage(m_offscreenImage, BORDER_PIXELS
					* m_scale, BORDER_PIXELS * m_scale, null);
	...
	/*
	 * public void toggleDialogMode() { m_dialogMode = !m_dialogMode;
	 * 
	 * if (m_dialogMode) { // Entering dialog mode m_spectrum.pause();
	 * 
	 * // Entering dialog mode removeKeyListener(m_spectrum.getKeyboard());
	 * addKeyListener(m_dialogModeKeyListener);
	 * 
	 * for (int addr16 = 0; addr16 < BaseMemory.PAGE_SIZE; addr16++) {
	 * m_screenBackup[addr16] = m_memory.read8(PIXEL_START + addr16); } } else {
	 * // Leaving dialog mode removeKeyListener(m_dialogModeKeyListener);
	 * addKeyListener(m_spectrum.getKeyboard());
	 * 
	 * // Restore screen contents via the memory object // in order to
	 * invalidate all addresses and mark // them for refresh for (int addr16 =
	 * 0; addr16 < BaseMemory.PAGE_SIZE; addr16++) { m_memory.write8(PIXEL_START
	 * + addr16, m_screenBackup[addr16]); }
	 * 
	 * m_spectrum.unpause(); }
	 * 
	 * repaint(); }
	...
	/**
	 * Construct the appropriate (derived class) instance for the current
	 * environment (JVM version.)
	 * <P>
	 * The method constructs an instance of <TT>ScreenJava1</TT> if the JVM is
	 * version 1.1.x, and an instance of <TT>ScreenJava2</TT> if the JVM is
	 * version of 1.2.x.
	 * 
	 * @see ScreenJava1
	 * @see ScreenJava2
