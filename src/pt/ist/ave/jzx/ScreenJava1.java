 ...
 * <P>
 * This implementation makes use of a cache of pre-rendered Image objects for a
 * given byte/attribute nibble. Its performance hinges on the fact that the call
 * to <TT>Graphics.drawImage()</TT> is very fast in JDK1.1.
 * 
...
public class ScreenJava1 extends BaseScreen {
	 ...
	 * <P>
	 * This is primarily for the purposes of running in Microsoft's JVIEW, since
	 * JVIEW has a limitation on the number of Image objects that can be
	 * allocated when running as an applet.
	 */
	 ...
	 * The image cache.
	 * <P>
	 * The cache is indexed on the 4 pixel bits and the 7 attribute bits
	 * (exclude the flash bit, since it's simply an inversion of the attribute.)
	 * In sum, the cache has 2^11 elements.
	 */
	 ...
	 * <P>
	 * If the cache is not yet full, a new image is created, initialized and
	 * placed into the cache. Otherwise, a random image is taken out of the
	 * cache, reinitialized and replaced into the cache in a different position.
	 */
	protected Image getImage(int pix4, int attr8) {
		...
		if (image == null) {
			...
			for (int i = 0; i < 4; i++) {
				graphics.setColor(s_colorPalette[((pix4 & 0x08) != 0 ? ipix
						: ppix)]);
	...
	/**
	 * Draw a given byte onto the screen using the given attribute, at the given
	 * location.
	 */
	protected void draw8(int x, int y, int pix8, int attr8) {
		...
		m_offscreenImageGraphics.drawImage(image, x * m_scale, y * m_scale,
				this);

		image = getImage(pix8 & 0x0f, attr8);
		m_offscreenImageGraphics.drawImage(image, (x + 4) * m_scale, y
				* m_scale, this);
	}
