 ...
 * <P>
 * This implementation makes use of a BufferedImage object, whose setRGB()
 * method is very fast for updating a large array of new RGB values.
 * 
...
public class ScreenJava2 extends BaseScreen {
	...
	/**
	 * Compute the size of the segment that will be drawn into the
	 * BufferedImage, and allocate it.
	 */
	...
	protected Image createCompatibleImage(int width, int height) {
		m_bufferedOffscreenImage = (BufferedImage) super.createCompatibleImage(
				width, height);
	...
	protected void draw8(int x, int y, int pix8, int attr8) {
		...
		/*
		 * 16bit: rmask=f800 gmask=7e0 bmask=1f amask=0 32bit: rmask=ff0000
		...
		for (int i = 1; i < m_pix8Height; i++) {
			System.arraycopy(segment, offset, segment, offset + i * width,
					m_pix8Width);
		}

		if (update) {
			m_bufferedOffscreenImage.setRGB(xscale, yscale, // starting
															// coordinates
	...
	public void dumpScreenshot() {
		try {
			// Save as PNG
			File file = new File("screenshot-" + System.currentTimeMillis()
					+ ".png");
			ImageIO.write(m_bufferedOffscreenImage, "png", file);
		} catch (IOException e) {
			m_logger.log(ILogger.C_ERROR,
